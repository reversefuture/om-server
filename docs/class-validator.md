...
let post = new Post();
post.title = 'Hello world!'; // allowed but not decorated with contraints
post.views = 420;

post.nonWhitelistedProperty = 69; // no decorator
(post as any).anotherNonWhitelistedProperty = "something"; // not defined
...
usually desirable when you want to **update some parts of the object, and want to validate only updated parts, but skip everything else, e.g. skip missing properties.**
In such situations you will need to pass a special flag to `validate` method:

```typescript
import { validate } from 'class-validator';
// ...
validate(post, { skipMissingProperties: true }); // {title:'updated title'} will only validate the title while skipping missing properties
```
